...
public:
	...
	/// Iterator to the current position of this frame in
	/// Module::alloc_accesses
	// LMStore specific
	std::list<Frame *>::iterator alloc_accesses_iterator;
	...
	/// Flag indicating whether a memory reference to lmstore is allocate.
	bool allocate = false;

	/// Flag indicating whether a memory reference to lmstore is deallocate.
	...
	/// Size in bytes of a block <-- lmstore specific
