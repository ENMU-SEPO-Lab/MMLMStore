...
public:
	...
	enum AccessType
	{
		...
		AccessStore=2,
		AccessNCStore=3,
		AccessReadAlloc=4,	// <--- LMStore specific
		AccessNoReadAlloc=5,      // <--- LMStore specific
		AccessWriteDealloc=6,	// <--- LMStore specofic
		AccessNoWriteDealloc=7
	};
...
private:
	...
	// List of all in-flight alloc accesses
	std::list<Frame *> alloc_accesses;
	...
	// Lmstore
	std::unique_ptr<LMStore> lmstore;
...
public:
	...
	// Statistics for lmstore
	long long num_allocate = 0;
	long long accumulated_sizes_alloc_blocks= 0;
	long long max_alloc_block_size = 0;
	long long min_alloc_block_size = 0;
	long long num_deallocate = 0;
	long long accumulated_sizes_dealloc_blocks= 0;
	long long max_dealloc_block_size = 0;
	long long min_dealloc_block_size = 0;
	long long num_lmstore_reads = 0;
	long long num_lmstore_writes = 0;
	long long num_free_lmref = 0;
	long long num_free_lmdir = 0;
	long long lmstore_free_space = 0;
	...
	/// return true if the block size is power of 2
	bool IsPowerOfTwo(unsigned size){ return (size != 0) && ((size & (size - 1)) == 0); }

	/// return block size after rounding it up to next power of 2
	unsigned long upper_power_of_two(unsigned long size)
	{
		size--;
		size |= size >> 1;
		size |= size >> 2;
	...
	void setBlockSize(unsigned size)
	{
		if(IsPowerOfTwo(size))
			...
			block_size = upper_power_of_two(size);
	}
	...
	/// Create a lmstore structure ith the module.
	void setLMStore(unsigned BlockSize,
			unsigned LMRefSize,
			unsigned LMDirSize,
			unsigned numBlocks,
			unsigned LMStoreSize)
	{
		assert(!lmstore.get());
		lmstore = 
			misc::new_unique<LMStore>(
				BlockSize,
				LMRefSize,
				LMDirSize,
				numBlocks,
				LMStoreSize);
		num_free_lmref = LMRefSize;
		num_free_lmdir = LMDirSize;
		lmstore_free_space = LMStoreSize;
		min_alloc_block_size = LMStoreSize;
		min_dealloc_block_size = LMStoreSize;
	}
	...
	/// Get the LMStore structure associated with the module, as previously
	/// created by a call to setLMStore(). If setLMStore() wasn't invoked
	/// before, return nullptr.
	LMStore *getLMStore() const { return lmstore.get(); }
	...
	/// Return the youngest in-flight alloc older than \a older_than_frame.
	/// If \a older_than_frame is `nullptr`, return the youngest in-flight
	/// write. Return `nullptr` if there is no in-flight alloc.
	Frame *getInFlightAlloc(Frame *older_than_frame = nullptr);
	...
	/// Return an iterator to the first element of the list of in-flight
	/// alloc accesses.
	std::list<Frame *>::iterator getAllocAccessListBegin()
	{
		return alloc_accesses.begin();
	}
	...
	/// Return a past-the-end iterator to the list of in-flight alloc 
	/// accesses.
	std::list<Frame *>::iterator getAllocAccessListEnd()
	{
		return alloc_accesses.end();
	}
