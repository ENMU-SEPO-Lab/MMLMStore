...
Module *System::ConfigReadLMStore(misc::IniFile *ini_file,
		const std::string &section)
{
	// LMStore parameters
	std::string geometry_section = "LMStoreGeometry " +
	...
	ini_file->Enforce(geometry_section, "LMRefSize");
	ini_file->Enforce(geometry_section, "LMDirSize");
	ini_file->Enforce(geometry_section, "LMStoreSize");
	ini_file->Enforce(geometry_section, "numBlocks");
	...
	// Geometry Values
	int BlockSize = ini_file->ReadInt(geometry_section, "BlockSize", 64);
	int LMRefSize = ini_file->ReadInt(geometry_section, "LMRefSize", 64);
	int LMDirSize = ini_file->ReadInt(geometry_section, "LMDirSize", 64);
	int LMStoreSize = ini_file->ReadInt(geometry_section, "LMStoreSize", 64);
	int NumBlocks = ini_file->ReadInt(geometry_section, "numBlocks", 64);
	int DataLatency = ini_file->ReadInt(geometry_section, "Latency", 1);
	int NumPorts = ini_file->ReadInt(geometry_section, "Ports", 2);
	int MSHRSize = ini_file->ReadInt(geometry_section, "MSHR", 16);

	// Check parameters
	if (BlockSize < 1 || (BlockSize & (BlockSize - 1)))
...
void System::ConfigReadL1Cache(misc::IniFile *ini_file)
{
	// Traverse modules
	for (auto &module : modules)
	{
		...
		std::string l1cache_name = 
			ini_file->ReadString(section,"L1Cache");

		if (l1cache_name.empty())
		...
		// Get l1cache and assign
		Module *l1cache_module = (Module *) ini_file->ReadPointer(section, "ptr");
		assert(l1cache_module);
		module->getLMStore()->setL1Cache(l1cache_module);
	}
}
...
void System::ConfigReadModules(misc::IniFile *ini_file)
{
			...
			++it)
	{
		...
		else if (!misc::StringCaseCompare(module_type, "LMStore"))
			module = ConfigReadLMStore(ini_file, section);
		...
		int level)
{
	...
	// Check block size
	//cout << module->getBlockSize() << " : " << block_size << endl;
...
void System::ReadConfiguration(misc::IniFile *ini_file)
{
	...
	// Read L1Cache for LMStore
	ConfigReadL1Cache(ini_file);
