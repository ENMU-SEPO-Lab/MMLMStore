...
namespace mem
{
	...
	{
		// LMStore has a different way to verify address
		if (type == TypeLMStore)
			return lmstore->ServesAddress(address);
	...
	Module *Module::getLowModuleServingAddress(unsigned address) const
	{
		...
		else if (type == TypeLMStore)
		{
			// Depends on the passed-in address
			if (lmstore->ServesAddress(address))
				return getLowModule(0); // return MainMemory
			else
				return getLMStore()->getL1Cache(); // return L1 cache
		}
		else
		{
			...
			esim::Event *return_event)
	{
		...
		if (type == TypeLMStore)
		{
			...
			{
				unsigned opcode = 
					module->getLMStore()->getOpCode(address);
				if (opcode == LMStore::BlockReadAlloc)
				{
					access_type = AccessReadAlloc;
				}
				else if (opcode == LMStore::BlockNoReadAlloc)
				{
					access_type = AccessNoReadAlloc;
				}
				else if (opcode == LMStore::BlockNoWriteDealloc)
				{
					access_type = AccessNoWriteDealloc;
				}
				else if (opcode == LMStore::BlockWriteDealloc)
				{
					access_type = AccessWriteDealloc;
				}
			}
			else
			{
				// Redirect this access request to L1 cache
				assert(getLMStore()->getL1Cache());
				module = getLMStore()->getL1Cache();
		...
		switch (module_type)
		{
			...
			case TypeMainMemory:
				...
				switch (access_type)
				{
					...
					default:
						...
						throw misc::Panic("Invalid cache access type");
				}
				break;
			...
			case TypeLMStore:
				...
				switch (access_type)
				{
					...
					case AccessReadAlloc:
					case AccessNoReadAlloc:
						event = System::event_lmstore_alloc;
						break;

					case AccessWriteDealloc:
					case AccessNoWriteDealloc:
						event = System::event_lmstore_dealloc;
						break;

					case AccessLoad:
						event = System::event_lmstore_load;
						break;

					case AccessStore:
						event = System::event_lmstore_store;
						break;
	...
	void Module::StartAccess(Frame *frame, AccessType access_type)
	{
		...
		// Insert in write access list
		if (access_type == AccessReadAlloc || access_type == AccessNoReadAlloc)
			frame->alloc_accesses_iterator = alloc_accesses.insert(
					alloc_accesses.end(),
					frame);
	...
	void Module::FinishAccess(Frame *frame)
	{
		...
		if (frame->access_type == Module::AccessReadAlloc ||
                    frame->access_type == Module::AccessNoReadAlloc )
		{
			alloc_accesses.erase(frame->alloc_accesses_iterator);
			frame->alloc_accesses_iterator = alloc_accesses.end();
		}
	...
	Frame *Module::getInFlightAlloc(Frame *older_than_frame)
	{
		// No 'older_than_frame' given, return youngest write, or nullptr if
		// there is no in-flight write.
		if (!older_than_frame)
			return alloc_accesses.size() ?
				alloc_accesses.back() :
				nullptr;
		...
		while (it != accesses.begin())
		{
			...
			// Check if it is a write
			Frame *frame = *it;
			if (frame->access_type == AccessReadAlloc || 
			    frame->access_type == AccessNoReadAlloc)
				return frame;
		}
	...
	void Module::DumpReport(std::ostream &os) const
	{
		...
		if (type == TypeLMStore)
		{
			// Dump LMStore allocated block information
			os << misc::fmt("[ %s.Allocate ]\n", name.c_str());
			os << misc::fmt("NumberOfBlocks = %lld\n", num_allocate);
			os << misc::fmt("SizeOfBlocksInBytes = %lld\n", accumulated_sizes_alloc_blocks);
			os << misc::fmt("MaximumBlockSizeInBytes = %lld\n", max_alloc_block_size);
			os << misc::fmt("MinimumBlockSizeInBytes = %lld\n", min_alloc_block_size);
			os << misc::fmt("AverageBlockSize = %0.2f\n", num_allocate ?
					(double) accumulated_sizes_alloc_blocks / num_allocate : 0.0);
			os << "\n";

			// Dump LMStore Deallocated block information
			os << misc::fmt("[ %s.Deallocate ]\n", name.c_str());
			os << misc::fmt("NumberOfBlocks = %lld\n", num_deallocate);
			os << misc::fmt("SizeOfBlocksInBytes = %lld\n", accumulated_sizes_dealloc_blocks);
			os << misc::fmt("MaximumBlockSizeInBytes = %lld\n", max_dealloc_block_size);
			os << misc::fmt("MinimumBlockSizeInBytes = %lld\n", min_dealloc_block_size);
			os << misc::fmt("AverageBlockSize = %0.2f\n", num_deallocate ?
					(double) accumulated_sizes_dealloc_blocks / num_deallocate : 0.0);
			os << "\n";

			// Dump LMStore Globals
			os << misc::fmt("[ %s.Globals ]\n", name.c_str());
			os << misc::fmt("LMStoreSize = %d\n", getLMStore()->getLMStoreSize());
			os << misc::fmt("FreeLMStore = %lld\n", lmstore_free_space);
			os << misc::fmt("LMRefSize = %d\n", getLMStore()->getLMRefSize());
			os << misc::fmt("FreeLMRefs = %lld\n", num_free_lmref);
			os << misc::fmt("LMDirSize = %d\n", getLMStore()->getLMDirSize());
			os << misc::fmt("FreeLMDirs = %lld\n", num_free_lmdir);
			os << "\n";
		}

		os << misc::fmt("[ %s.MemoryAccess ]\n", name.c_str());

		if (type == TypeLMStore)
		{

			// Dump LMStore memory access information
			os << misc::fmt("NumberOfLMStoreReads = %lld\n", num_lmstore_reads);
			os << misc::fmt("NumberOfLMStoreWrites = %lld\n", num_lmstore_writes);
			os << "\n";
		}
		// Statistics - Accesses
	...
	void Module::UpdateStats(Frame *frame)
	{
		...
		if (frame->request_direction == Frame::RequestDirectionUpDown)
		{
			if (frame->read)
			{
				...
				if (this->getType() == Module::TypeLMStore 
						/*&& frame->allocate == false 
						&& frame->deallocate == false*/)
				num_lmstore_reads++;
			}
			...
			else if(frame->allocate)
			{
					num_allocate++;
					accumulated_sizes_alloc_blocks += frame->block_size;

					if(min_alloc_block_size > frame->block_size)
						min_alloc_block_size = frame->block_size;     
					
					if (max_alloc_block_size < frame->block_size)
						max_alloc_block_size = frame->block_size;
					num_free_lmref--;
					num_free_lmdir--;
					lmstore_free_space-= frame->block_size;

			}
			else if(frame->deallocate)
			{

				/*cout << "num_deallocate : " << num_deallocate <<
					"block_size : " << frame->block_size <<
					"accumulated_sizes_dealloc_blocks : " <<
					accumulated_sizes_dealloc_blocks <<
					"lmstore_free_space : " << lmstore_free_space << "\n";*/
					num_deallocate++;
					accumulated_sizes_dealloc_blocks += frame->block_size;

					if(min_dealloc_block_size > frame->block_size)
						min_dealloc_block_size = frame->block_size;     

					if (max_dealloc_block_size < frame->block_size)
						max_dealloc_block_size = frame->block_size;
					num_free_lmref++;
					num_free_lmdir++;
					lmstore_free_space+= frame->block_size;
			}
