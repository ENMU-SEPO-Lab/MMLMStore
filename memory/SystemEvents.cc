...
namespace mem
{
	...
	// ======================== LMStore Events =====================
	esim::Event *System::event_lmstore_load;
	esim::Event *System::event_lmstore_load_lock;
	esim::Event *System::event_lmstore_load_action;
	esim::Event *System::event_lmstore_load_unlock;
	esim::Event *System::event_lmstore_load_finish;

	esim::Event *System::event_lmstore_store;
	esim::Event *System::event_lmstore_store_lock;
	esim::Event *System::event_lmstore_store_action;
	esim::Event *System::event_lmstore_store_unlock;
	esim::Event *System::event_lmstore_store_finish;

	esim::Event *System::event_lmstore_find_and_lock;
	esim::Event *System::event_lmstore_find_and_lock_port;
	esim::Event *System::event_lmstore_find_and_lock_action;
	esim::Event *System::event_lmstore_find_and_lock_finish;

	esim::Event *System::event_lmstore_alloc;
	esim::Event *System::event_lmstore_alloc_lock;
	esim::Event *System::event_lmstore_alloc_action;
	esim::Event *System::event_lmstore_alloc_process;
	esim::Event *System::event_lmstore_alloc_unlock;
	esim::Event *System::event_lmstore_alloc_finish;

	esim::Event *System::event_lmstore_dealloc;
	esim::Event *System::event_lmstore_dealloc_lock;
	esim::Event *System::event_lmstore_dealloc_action;
	esim::Event *System::event_lmstore_dealloc_process;
	esim::Event *System::event_lmstore_dealloc_unlock;
	esim::Event *System::event_lmstore_dealloc_finish;
			...
			esim::Frame *esim_frame)
	{
		...
		if (event == event_write_request_action)
		{
			...
			if (frame->getModule()->getType() == Module::TypeLMStore)
			{
				debug << misc::fmt("  %lld A-%lld 0x%x %s "
					"write_request_action skip Invalidate event\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->tag,
					target_module->getName().c_str());

				esim_engine->Next(event_write_request_exclusive);
				return;
			}	
		...
		if (event == event_write_request_updown_finish)
		{
			...
			debug << misc::fmt("  %lld A-%lld 0x%x %s (latency = %d) event_write_request_updown_finish\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->tag,
					target_module->getName().c_str(),
		...
		if (event == event_write_request_downup_finish)
		{
			...
			switch (parent_frame->reply)
			{
				...
				case Frame::ReplyAckData:
					{
						// Data latency
						debug << misc::fmt("  %lld A-%lld 0x%x %s (latency = %d)"
								"write_request_downup_finish\n",
								esim_engine->getTime(),
								frame->getId(),
								frame->tag,
			...
			esim::Frame *esim_frame)
	{
		...
		if (event == event_read_request_updown)
		{
			...
			if (frame->state)
			{
				...
				//
				// Red flag!!!!! lmstore as module has getBlockSize?
				//assert(frame->getAddress() % module->getBlockSize() == 0);
				Directory *directory = target_module->getDirectory();
				for (int z = 0; z < directory->getNumSubBlocks(); z++)
				{
					...
					// Red flag!!!!! lmstore as module has getIndex?
		...
		if (event == event_read_request_reply)
		{
			...
			{
				frame->reply_size = module->getBlockSize();
				debug << misc::fmt("net.msg_send "
						"src=\"%s\" "
						"des=\"%s\" (size=%d)\n",
						source_node->getName().c_str(),
			...
			esim::Frame *esim_frame)
	{
		...
		if (event == event_invalidate)
		{
			...
			debug << misc::fmt("  %lld A-%lld 0x%x %s invalidate "
					...
					frame->way,
					Cache::BlockStateMap[frame->state]);
			debug << misc::fmt("  %lld A-%lld 0x%x %s invalidate -- directory details ---"
					" (NumSubBlocks=%d getNumNodes=%d)\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->tag,
					module->getName().c_str(),
					directory->getNumSubBlocks(),
					directory->getNumNodes());
			trace << misc::fmt("mem.access name=\"A-%lld\" "
					"state=\"%s:invalidate\"\n",
					frame->getId(),
					module->getName().c_str());
			...
			for (int z = 0; z < directory->getNumSubBlocks(); z++)
			{
				...
				assert(directory_entry_tag < frame->tag +
						(unsigned) module->getBlockSize());
				debug << misc::fmt("  %lld A-%lld 0x%x %s invalidate --> Skip other sub-blocks?",
						esim_engine->getTime(),
						frame->getId(),
						frame->tag,
						module->getName().c_str());
				...
				{
					debug << misc::fmt("  %lld A-%lld 0x%x %s invalidate --> Skip non-sharers?",
							esim_engine->getTime(),
							frame->getId(),
							frame->tag,
							module->getName().c_str());
					...
					debug << misc::fmt("  %lld A-%lld 0x%x %s invalidate --> Skip 'except_module'?\n",
							esim_engine->getTime(),
							frame->getId(),
							frame->tag,
							module->getName().c_str());
					...
					{
						debug << misc::fmt("  %lld A-%lld 0x%x %s invalidate --> setOwner()\n",
								esim_engine->getTime(),
								frame->getId(),
								frame->tag,
								module->getName().c_str());
					...
					debug << misc::fmt("  %lld A-%lld 0x%x %s invalidate --> Skip mid-block sub-blocks?\n",
							esim_engine->getTime(),
							frame->getId(),
							frame->tag,
							module->getName().c_str());
					...
					new_frame->request_direction = Frame::RequestDirectionDownUp;
					debug << misc::fmt("  %lld A-%lld 0x%x module %s target_module %s invalidate --> Call event_write_request\n",
							esim_engine->getTime(),
							frame->getId(),
							frame->tag,
							module->getName().c_str(),
							sharer->getName().c_str());
	...
	void System::EventLMStoreLoadHandler(esim::Event *event,
			esim::Frame *esim_frame)
	{
		// Get useful objects
		esim::Engine *esim_engine = esim::Engine::getInstance();
		...
		// double check
		//assert(module->lmstore->ServesAddress(frame->getAddress()));

		// Event "lmstore_load"
		if (event == event_lmstore_load)
		{
			// lmstore debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_load\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			module->incCoalescedReads();
			module->Coalesce(master_frame, frame);
			master_frame->queue.Wait(event_lmstore_load_finish);
...
*/
			// Continue
			esim_engine->Next(event_lmstore_load_lock);

			return;

		}

		// Event "lmstore_load_lock"
		if (event == event_lmstore_load_lock)
		{
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_load_lock\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			// If there is any older write, wait for it
			Frame *older_frame = module->getInFlightAlloc(frame);
			if (older_frame)
			{
				debug << misc::fmt("    A-%lld wait for alloc A-%lld\n",
						frame->getId(),
						older_frame->getId());
				older_frame->queue.Wait(event_lmstore_load_lock);
				return;
			}

			// If there is any older write, wait for it
			older_frame = module->getInFlightWrite(frame);
			if (older_frame)
			{
				debug << misc::fmt("    A-%lld wait for write A-%lld\n",
						frame->getId(),
			...
			new_frame->read = true;
			new_frame->retry = frame->retry;
			esim_engine->Call(event_lmstore_find_and_lock,
					new_frame,
					event_lmstore_load_action);

			return;
		}

		// Event "lmstore_load_action"
		if (event == event_lmstore_load_action)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s frame->state 0x%x lmstore_load_action\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str(),
			...
			if (frame->error)
			{
				...
				// Reschedule 'lmstore_load-lock'
				frame->retry = true;
				esim_engine->Next(event_lmstore_load_lock, retry_latency);
				return;
			}

			// Should always result in hit
			//assert(frame->state);
			assert(frame->hit);

			// Hit! 
			// Continue with 'lmstore_load-unlock'
			esim_engine->Next(event_lmstore_load_unlock);
			return;

		}

		// Event "lmstore_load_unlock"
		if (event == event_lmstore_load_unlock)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s (latency = %d)"
					"lmstore load unlock\n",
			...
			// Continue with 'lmstore_load-finish' after latency
			esim_engine->Next(event_lmstore_load_finish,
					module->getDataLatency());
			return;
		}

		// Event "lmstore_load_finish"
		if (event == event_lmstore_load_finish)
		{
			// Memory debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_load_finish\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
	...
	void System::EventLMStoreStoreHandler(esim::Event *event,
			esim::Frame *esim_frame)
	{
		// Get useful objects
		esim::Engine *esim_engine = esim::Engine::getInstance();
		...
		Frame *frame = misc::cast<Frame *>(esim_frame);
		Module *module = frame->getModule();
		LMStore *lmstore = module->getLMStore();

		// Event "lmstore_store"
		if (event == event_lmstore_store)
		{
			// lmstore debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_store\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			module->incCoalescedWrites();
			module->Coalesce(master_frame, frame);
			master_frame->queue.Wait(event_lmstore_store_finish);
...
*/
			// Continue
			esim_engine->Next(event_lmstore_store_lock, 0);
			return;
		}

		// Event "lmstore_store_lock"
		if (event == event_lmstore_store_lock)
		{
			// Debug
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_store_lock\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			if (it != module->getAccessListBegin())
			{
				...
				// Enqueue
				older_frame->queue.Wait(event_lmstore_store_lock);
				return;
			}
		...
		// Event "lmstore_store_action"
		if (event == event_lmstore_store_action)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_store_action\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			if (frame->error)
			{
				...
				// Reschedule 'store-lock' after lantecy
				frame->retry = true;
				esim_engine->Next(event_lmstore_store_lock, retry_latency);
				return;
			}
			...
			assert(frame->hit);
			// Update block state
			lmstore->setBlockState(frame->getAddress(),
					LMStore::BlockModified);


			// Continue with 'lmstore_store-unlock'
			esim_engine->Next(event_lmstore_store_unlock);
			return;
		}

		// Event "lmstore_store_unlock"
		if (event == event_lmstore_store_unlock)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s (latency = %d) lmstore_store_unlock\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str(),
			...
			trace << misc::fmt("mem.access "
					"name=\"A-%lld\" "
					"state=\"%s:lmstore_store_unlock\"\n",
					frame->getId(),
					module->getName().c_str());

			// Continue to 'store-finish' after data latency
			module->incDataAccesses();
			esim_engine->Next(event_lmstore_store_finish,
					module->getDataLatency());
			return;
		}

		// Event "lmstore_store_finish"
		if (event == event_lmstore_store_finish)
		{
			// Debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_store_finish\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
	...
	void System::EventLMStoreFindAndLockHandler(esim::Event *event,
			esim::Frame *esim_frame)
	{
		// Get useful objects
		esim::Engine *esim_engine = esim::Engine::getInstance();
		...
		// Event "lmstore_find_and_lock"
		if (event == event_lmstore_find_and_lock)
		{
			debug << misc::fmt("  %lld A-%lld 0x%x %s "
					"lmstore_find_and_lock (blocking=%d)\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str(),
			...
			// Get a port
			module->LockPort(frame, event_lmstore_find_and_lock_port);

			return;
		}

		// Event "lmstore_find_and_lock_port"
		if (event == event_lmstore_find_and_lock_port)
		{
			// Get locked port
			Module::Port *port = frame->port;
			assert(port);
			...
			// Memory debug
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_find_and_lock_port\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			// Look for block
			unsigned blockstate = (unsigned) frame->state;
			frame->hit = lmstore->FindBlock(frame->getAddress(), blockstate);
			if (blockstate == 0)
				frame->state = Cache::BlockInvalid;
			else
				frame->state = Cache::BlockExclusive;

			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore->FindBlock(), Hit = %s\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str(),
					frame->hit ? "True" : "False");
			...
			if (frame->hit)
			{
				debug << misc::fmt("    A-%lld 0x%x %s "
						...
						frame->set,
						frame->way,
						LMStore::BlockStateMap[frame->state]);
			...
			// Schedule event
			esim_engine->Next(event_lmstore_find_and_lock_action,
					module->getDataLatency());

			return;
		}

		// Event "lmstore_find_and_lock_action"
		if (event == event_lmstore_find_and_lock_action)
		{
			// Get locked port
			Module::Port *port = frame->port;
			assert(port);
			...
			// Schedule event
			esim_engine->Next(event_lmstore_find_and_lock_finish, 0);
			return;
		}

		// Event "lmstore_find_and_lock_finish"
		if (event == event_lmstore_find_and_lock_finish)
		{
			// Memory debug
			debug << misc::fmt("  %lld A-%lld 0x%x %s "
					"lmstore_find_and_lock_finish\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
	...
	void System::EventLMStoreAllocHandler(esim::Event *event,
			esim::Frame *esim_frame)
	{
		// Get useful objects
		esim::Engine *esim_engine = esim::Engine::getInstance();
		...
		// Ensure the block we are about to allocate does not exist
		unsigned blockstate = (unsigned) frame->state;
		assert(lmstore->FindBlock(frame->getAddress(), blockstate) == 0);

		// Event "lmstore_alloc"
		if (event == event_lmstore_alloc)
		{
			// lmstore debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_alloc\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			trace << misc::fmt("mem.new_access "
					"name=\"A-%lld\" "
					"type=\"alloc\" "
					"state=\"%s:alloc\" addr=0x%x\n",
					frame->getId(),
					module->getName().c_str(),
					frame->getAddress());
			// Record access
			module->StartAccess(frame, frame->access_type);
			...
			if (master_frame)
			{
				...
				module->incCoalescedReads();
				module->Coalesce(master_frame, frame);
				master_frame->queue.Wait(event_lmstore_alloc_finish);
			...
			// Continue
			esim_engine->Next(event_lmstore_alloc_lock);

			return;

		}

		// Event "lmstore_alloc_lock"
		if (event == event_lmstore_alloc_lock)
		{
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_alloc_lock\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			if (older_frame)
			{
				debug << misc::fmt("    A-%lld wait for write A-%lld\n",
						frame->getId(),
						older_frame->getId());
				older_frame->queue.Wait(event_lmstore_alloc_lock);
				return;
			}
			...
			// Continue
			esim_engine->Next(event_lmstore_alloc_action);
			return;
		}

		// Event "lmstore_alloc_action"
		if (event == event_lmstore_alloc_action)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_alloc_action\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			trace << misc::fmt("mem.access name=\"A-%lld\" "
					"state=\"%s:lmstore_alloc_action\"\n",
					frame->getId(),
					module->getName().c_str());

			frame->allocate = true;
			// Update frame block size
			frame->block_size = lmstore->getBlockSize(frame->getAddress());
			cout << "frame->block_size: " << frame->block_size
				<< "op: " << frame->access_type << endl;
			frame->request_direction = Frame::RequestDirectionUpDown;

			if (frame->access_type == Module::AccessNoReadAlloc)
			{
				frame->pending = 1;
				esim_engine->Next(event_lmstore_alloc_process);
			}
			else
			{
				// Initiate a read request to memory
				Module *mm = module->getLowModule(0);
				unsigned tag = lmstore->getBlockID(frame->getAddress());

				// Debug
				debug << misc::fmt("%s->getBlockSize(64) (size = %d)\n",
						module->getName().c_str(),
						module->getBlockSize());
				...
				//frame->block_size = lmstore->getBlockSize(frame->getAddress());
				int mmBlocks = 1;
				if (frame->block_size > mm->getBlockSize())
				{
					mmBlocks = frame->block_size / mm->getBlockSize();
					if (!(frame->block_size % mm->getBlockSize()))
						mmBlocks++;
				}
				...
				for (int i = 0; i < mmBlocks; i++)
				{
					...
					auto new_frame = misc::new_shared<Frame>(
							frame->getId(),
							module,
							tag + (mm->getBlockSize() * i));
					new_frame->target_module = mm;
					new_frame->request_direction = Frame::RequestDirectionUpDown;
					esim_engine->Call(event_read_request,
							new_frame,
							event_lmstore_alloc_process);
				}
				debug << misc::fmt("  %lld A-%lld 0x%x %s pending 0x%x --"
						"Done lmstore_alloc_action\n",
						esim_engine->getTime(),
						frame->getId(),
						frame->getAddress(),
						module->getName().c_str(),
		...
		// Event "lmstore_alloc_process"
		if (event == event_lmstore_alloc_process)
		{
			// Ignore while pending requests
			assert(frame->pending > 0);
			frame->pending--;
			if (frame->pending)
				return;
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_alloc_process\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			if (frame->error)
			{
				...
				// Continue with 'lmstore alloc-lock' after retry latency
				frame->retry = true;
				esim_engine->Next(event_lmstore_alloc_lock, retry_latency);
				return;
			}

			debug << misc::fmt("  %lld A-%lld 0x%x %s "
					"lmstore allocateBlock()\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());

			lmstore->allocateBlock(frame->getAddress());
			esim_engine->Next(event_lmstore_alloc_unlock);
			return;
		}

		// Event "lmstore_alloc_unlock"
		if (event == event_lmstore_alloc_unlock)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s (latency = %d)"
					"lmstore alloc_unlock\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str(),
			...
			// Continue with 'lmstore_alloc-finish' after latency
			esim_engine->Next(event_lmstore_alloc_finish,
					module->getDataLatency());
			return;
		}

		// Event "lmstore_alloc_finish"
		if (event == event_lmstore_alloc_finish)
		{
			// Memory debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_alloc_finish\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
	...
	void System::EventLMStoreDeallocHandler(esim::Event *event,
			esim::Frame *esim_frame)
	{
		// Engine, frame, module
		esim::Engine *esim_engine = esim::Engine::getInstance();
		...
		Module *module = frame->getModule();
		Module *mm = module->getLowModule(0);
		frame->target_module = mm;
		Module *target_module = frame->target_module;
		LMStore *lmstore = module->getLMStore();

		// Event 'lmstore_dealloc'
		if (event == event_lmstore_dealloc)
		{
			// lmstore debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_dealloc\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			// lmstore trace 
			trace << misc::fmt("%lld A-%lld 0x%x %s lmstore_dealloc\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			module->incCoalescedReads();
			module->Coalesce(master_frame, frame);
			master_frame->queue.Wait(event_lmstore_dealloc_finish);
			...
			*/
			...
			// Continue
			esim_engine->Next(event_lmstore_dealloc_lock);

			return;
		}

		// Event "lmstore_dealloc_lock"
		if (event == event_lmstore_dealloc_lock)
		{
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_dealloc_lock\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			...
			if (older_frame)
			{
				debug << misc::fmt("    A-%lld wait for write A-%lld\n",
						frame->getId(),
						older_frame->getId());
				older_frame->queue.Wait(event_lmstore_dealloc_lock);
				return;
			}
			...
			// Continue
			esim_engine->Next(event_lmstore_dealloc_action);

			return;
		}

		// Event "lmstore_dealloc_action"
		if (event == event_lmstore_dealloc_action)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_dealloc_action\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			trace << misc::fmt("mem.access name=\"A-%lld\" "
					"state=\"%s:lmstore_dealloc_action\"\n",
					frame->getId(),
					module->getName().c_str());

			frame->deallocate = true;
			// Update frame block size
			frame->block_size = lmstore->getBlockSize(frame->getAddress());
			frame->request_direction = Frame::RequestDirectionUpDown;
			cout << "Dealloc frame->access_type: " << frame->access_type
				<< "frame->block_size: " << frame->block_size << endl;
			if (frame->state != Cache::BlockModified ||
			    frame->access_type == Module::AccessNoWriteDealloc)
			{
				// No write-to-memory needed
				lmstore->deallocateBlock(frame->getAddress());
				esim_engine->Next(event_lmstore_dealloc_unlock);
				return;
			}

			// Initiate a write request to memory
			Module *mm = module->getLowModule(0);
			unsigned tag = lmstore->getBlockID(frame->getAddress());
			...
			for (int i = 0; i < mmBlocks; i++)
			{
				...
				// Set the expected reply size. This might change during the
				// down up write process, and invalidation
				new_frame->reply_size = mm->getBlockSize() + 8;
				new_frame->setReplyIfHigher(Frame::ReplyAckData);

				esim_engine->Call(event_write_request,
						new_frame,
						event_lmstore_dealloc_process);
			}
		...
		// Event "lmstore_dealloc_process"
		if (event == event_lmstore_dealloc_process)
		{
			// Ignore while pending requests
			assert(frame->pending > 0);
			frame->pending--;
			if (frame->pending)
				return;
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s lmstore_dealloc_process\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
			trace << misc::fmt("mem.access "
					"name=\"A-%lld\" "
					"state=\"%s:lmstore_dealloc_process\"\n",
					frame->getId(),
					module->getName().c_str());
			...
			if (frame->error)
			{
				...
				// Continue with 'lmstore dealloc-lock' after retry latency
				frame->retry = true;
				esim_engine->Next(event_lmstore_dealloc_lock, retry_latency);
				return;
			}

			debug << misc::fmt("  %lld A-%lld 0x%x %s "
					"lmstore DeallocateBlock()\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
		...
		// Event "lmstore_dealloc_unlock"
		if (event == event_lmstore_dealloc_unlock)
		{
			// Debug and trace
			debug << misc::fmt("  %lld A-%lld 0x%x %s (latency = %d)"
					"lmstore_dealloc_unlock\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str(),
			...
			trace << misc::fmt("mem.access "
					"name=\"A-%lld\" "
					"state=\"%s:lmstore_dealloc_unlock\"\n",
					frame->getId(),
					module->getName().c_str());
			...
			//lmstore->deallocateBlock(frame->getAddress());
			module->UpdateStats(frame); 
			esim_engine->Next(event_lmstore_dealloc_finish);
			return;
		}

		// Event "lmstore_dealloc_finish"
		if (event == event_lmstore_dealloc_finish)
		{
			// Memory debug
			debug << misc::fmt("%lld A-%lld 0x%x %s lmstore_dealloc_finish\n",
					esim_engine->getTime(),
					frame->getId(),
					frame->getAddress(),
					module->getName().c_str());
